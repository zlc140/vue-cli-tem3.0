    ...
    function ajax(option) {
        ...
        var xhr = {
            responseType: 'text'
        }, url = option.url, sendData = option.data, cordova = window.cordova;
        //检查请求协议  避免在本地请求出错
        if (url_1.protocol(option.url) === 'file:') {
            //检查是否cordova环境
                ...
                if (xhr.status === 200) {
                    switch (option.dataType || 'json') {
                        ...
                        case 'xml':
                            ...
                            break;
                        case 'json':
                            res = json_1.parse(xhr.responseText);
                            break;
                        default:
        ...
        switch (option.type) {
            ...
            case 'FORM':
                ...
                break;
            case 'GET':
                url = url_1.computedUrl(url, option.data);
                break;
            case 'DELETE':
                break;
            case 'PUT':
        ...
        switch (option.type) {
            case 'POST':
                sendData = json_1.stringify(sendData);
                break;
            case 'FORM':
