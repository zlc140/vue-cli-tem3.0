...
// 存储jsonp处理中的数据
let recordJsonpStroage = {};
// jsonp数据缓存对象 (哪个请求先回调也就哪个请求先加载完毕 ,也就是哪个先写入缓存哪个就先得到缓存)
let cssElement = window.document.createElement('link');
let jsElement = window.document.createElement('script');
let headElement = window.document.getElementsByTagName('head')[0] || window.document.documentElement;
...
function getJs(option) {
  // 回调处理
  let callbackHandle = function (resInfo) {
    let resData = resInfo.many ? resInfo.resData : resInfo.resData[0];
  ...
  let callback = option.jsonpCallback;
  let done = false;
  let scriptNode = jsElement.cloneNode();
